...
<header class="topbar">
  ...
  <form class="search-form" class:focused={searchFocused} onsubmit={handleSearch}>
    <div class="search-icon"><Icon icon="lucide:search" width="16" height="16" /></div>
  ...
  <div class="topbar-actions">
    ...
    {#if user}
      ...
      <a href="/submit" class="topbar-create-btn" aria-label="Create post">
        <Icon icon="lucide:plus" width="14" height="14" /><span>Ask</span>
      </a>
      ...
      <div class="avatar-menu-wrapper">
        <button class="avatar-btn" onclick={toggleAvatarMenu} aria-label="User menu">
          ...
          {/if}
          <span class="avatar-caret" class:open={avatarMenuOpen}>
        ...
        {#if avatarMenuOpen}
          <div class="avatar-dropdown">
            ...
            <a href="/u/{user.username}" class="dropdown-item" onclick={closeMenu}>
              <Icon icon="lucide:user" width="16" height="16" />Profile
            </a>
            <a href="/bookmarks" class="dropdown-item" onclick={closeMenu}>
              <Icon icon="lucide:bookmark" width="16" height="16" />Bookmarks
            </a>
            <a href="/settings" class="dropdown-item" onclick={closeMenu}>
              <Icon icon="lucide:settings" width="16" height="16" />Settings
            </a>
            <div class="dropdown-divider"></div>
            <button class="dropdown-item danger" onclick={() => { signOut(); closeMenu(); }}>
              <Icon icon="lucide:log-out" width="16" height="16" />Sign out
            </button>
          </div>
        {/if}
      </div>
...
<style>
  ...
  .search-form.focused { border-color: var(--accent); background: var(--surface); box-shadow: 0 0 0 3px var(--accent-subtle); }
  ...
  .theme-toggle:hover { background: var(--surface-raised); color: #f59e0b; }
  ...
  .icon-btn:hover { background: var(--surface-raised); color: var(--accent); }
  ...
  .topbar-create-btn { display: flex; align-items: center; gap: 0.375rem; padding: 0.4rem 0.875rem; background: linear-gradient(135deg, var(--accent) 0%, var(--accent-dark) 100%); color: white; border-radius: 10px; text-decoration: none; font-size: 0.875rem; font-weight: 600; transition: all 0.15s ease; box-shadow: 0 2px 8px var(--accent-shadow); }
  .topbar-create-btn:hover { background: var(--accent-dark); transform: translateY(-1px); box-shadow: 0 4px 12px var(--accent-shadow); }
  ...
  .avatar-img { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border); }
  .avatar-initials { width: 32px; height: 32px; background: linear-gradient(135deg, var(--accent) 0%, var(--accent-light) 100%); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 700; border: 2px solid var(--surface); }
  ...
  .avatar-dropdown { position: absolute; top: calc(100% + 8px); right: 0; min-width: 200px; background: var(--surface); border: 1px solid var(--border); border-radius: 14px; box-shadow: 0 8px 32px rgba(0,0,0,0.12); overflow: hidden; animation: dropdownIn 0.15s ease-out; }
  ...
  .dropdown-item:hover { background: var(--surface-blue); color: var(--accent); }
  .dropdown-item.danger:hover { background: var(--surface-red); color: #ef4444; }
  ...
  .auth-btn.ghost { color: var(--text-primary); border: 1.5px solid var(--border); background: var(--surface); }
  .auth-btn.ghost:hover { border-color: var(--accent); background: var(--surface-blue); }
  .auth-btn.solid { background: linear-gradient(135deg, var(--accent) 0%, var(--accent-dark) 100%); color: white; box-shadow: 0 2px 8px var(--accent-shadow); }
  .auth-btn.solid:hover { background: var(--accent-dark); transform: translateY(-1px); }
